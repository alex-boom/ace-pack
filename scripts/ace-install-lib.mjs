...
import { onboardRepository } from './ace-onboard.mjs'
import {
...
async function ensureIdeRuleBridges(rootDir, packageManager) {
  ...
  for (const relativePath of IDE_BRIDGE_FILES) {
    ...
    await writeFile(filePath, normalizeTrailingNewline(nextResult.content), 'utf8')
    if (existingContent === null) {
...
function stripByteOrderMark(content) { return content.replace(/^\uFEFF/u, '') }
export async function installAcePack(targetDir) {
  ...
  updatedFiles.push(...bridgeResult.updatedFiles)
...
function formatPowerShellPnpmCommand(command) { return command.replace(/^pnpm /u, 'pnpm.cmd ') }
