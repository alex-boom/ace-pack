...
- Added canonical v2 memory path definitions, legacy alias resolution, mirror
  writes, and schema v2 migration helpers.
...
- Writes brief reports to `.ai/generated/report-brief.md` and mirrors the
  legacy `.ai/report-brief.md` path.

[scripts/ai-report.mjs]
- Writes full reports and XML bundles under `.ai/generated/**` while preserving
  legacy report mirrors.

[scripts/ace-hub.mjs]
- Writes generated hub context to `.ai/generated/context.md` and mirrors
  `.ai/generated-context.md`.
- Displays canonical v2 context paths in generated headers.

[scripts/ace-onboard.mjs]
- Writes profile and recommended config under `.ai/config/**` with legacy
  mirrors.
...
- Writes task, handoff, decision, work-log, and changed-file updates to
  canonical v2 paths and legacy mirrors.
...
- Replaced the v1 contract with the v2 command/router, canonical memory layout,
  legacy mirror, and migration policy contract.
...
- Added canonical generated report and legacy mirror assertions.
...
- Applied schema v2 dogfood migration to canonical `.ai/config`, `.ai/state`,
  `.ai/knowledge`, and `.ai/generated` categories while preserving legacy root
  mirrors.
