...
- `ace discover` performs a bounded local scan with simple path, dependency,
  and import-string heuristics. It writes an ACE-managed
  `.ai/knowledge/project-conventions.md` summary only when invoked, protects
  human-written conventions files unless `--force` is used, and keeps output
  aggregated for `ace hub` context safety.
...
- `ace classify` uses `ai-task-scope.mjs` for local git diff inputs. The
  default scope remains the full working tree, while `--staged` reads only the
  cached diff and repeated `--path` / comma-separated `--paths` limit
  classification to selected working-tree pathspecs. Scoped classification still
  runs configured path and keyword risk rules, and it rejects ambiguous
  combinations with PR refs.
- `ace finish` can auto-close small low-risk changes by writing compact
  ...
  git/classification data. It supports the same `--staged` and `--path` scope
  flags as `ace classify`, records scope in task-state/work-log summaries, and
  still keeps stricter closeout for standard, large, high-risk, and
  design-review-required work.
...
- Project conventions are stored in `.ai/knowledge/project-conventions.md` with
  `.ai/project-conventions.md` as a legacy read alias. The file is generated by
  `ace discover`, not by install or migration.
- Scoped fast-fix behavior adds no persistent schema fields. Classification
  scope is ephemeral CLI/JSON output and small-task closeout text.
...
- `2.2.0` discover performs no AI calls, network calls, package installs, or
  AST parsing. It reads bounded local source samples and manifest files only.
